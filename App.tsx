    ...
    <div className="min-h-screen bg-gray-100 flex flex-col font-sans">
      ...
      <nav className="bg-white border-b px-6 py-4 flex justify-between items-center no-print sticky top-0 z-50">
        ...
        <div className="flex gap-2 md:gap-3">
          <button onClick={() => setShowEditor(!showEditor)} className="md:hidden px-3 py-2 text-sm font-medium bg-gray-100 rounded">
          ...
          <button onClick={handleDownloadPDF} className="px-3 md:px-4 py-2 text-sm font-medium text-gray-700 bg-white border rounded hover:bg-gray-50">
            PDF
          </button>
          <button onClick={handlePrint} className="hidden md:flex px-4 py-2 text-sm font-medium text-white bg-amber-800 rounded hover:bg-amber-900">
            Print
          </button>
        </div>
      </nav>
      ...
      <main className="flex-1 max-w-[1600px] w-full mx-auto p-4 md:p-8 flex flex-col md:flex-row gap-8 overflow-hidden">
        ...
        {/* Preview Container */}
        <div className={`flex-1 overflow-auto flex justify-center items-start p-2 md:p-0 ${!showEditor ? 'flex' : 'hidden md:flex'}`}>
          <div className="w-full md:w-auto overflow-x-auto flex justify-center">
            <div id="invoice-pdf-content" className="bg-white shadow-2xl print:shadow-none origin-top transform scale-[0.45] sm:scale-[0.55] md:scale-100 min-w-[210mm]">
